...
class EmiliaTokenizer(Tokenizer):
    def __init__(self, token_file: Optional[str] = None, token_type="phone"):
        ...
        self.mongolian_normalizer = MongolianTextNormalizer()
        ...
        for text in texts:
            ...
            for index in range(len(segments)):
                ...
                if seg[1] == "zh":
                    ...
                    phoneme = self.tokenize_MN(seg[0])
                elif seg[1] == "pinyin":
    ...
    def tokenize_MN(self, text: str) -> List[str]:
        """Tokenize Mongolian Cyrillic text using espeak 'mn' phonemizer."""
        try:
            text = self.mongolian_normalizer.normalize(text)
            # Try espeak mn first
            try:
                tokens = phonemize_espeak(text, "mn")
                tokens = reduce(lambda x, y: x + y, tokens)
                if tokens:
                    return tokens
            except Exception:
                pass
            # Fallback: character-level (SimpleTokenizer style)
            # This ensures Mongolian Cyrillic chars are preserved as-is
            return list(text)
        except Exception as ex:
            logging.warning(f"Tokenization of Mongolian texts failed: {ex}")
            return list(text)
    ...
    def is_mongolian(self, char: str) -> bool:
        """Mongolian Cyrillic: А-Я + Өө Үү (U+04E8, U+04E9, U+04AE, U+04AF)"""
        if "\u0400" <= char <= "\u04FF":
            return True
        return False
