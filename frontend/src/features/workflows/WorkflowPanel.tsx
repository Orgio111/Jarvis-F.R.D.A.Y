...
import {
  Workflow, Plus, Play, Save, GripVertical, ChevronRight,
...
export function WorkflowPanel() {
  ...
  const [tab, setTab] = useState<'builder' | 'engine'>('builder');

  const TabButton = ({ id, label, icon: TabIcon }: { id: 'builder' | 'engine'; label: string; icon: React.ComponentType<{ size?: number | string }> }) => (
    <button
      onClick={() => setTab(id)}
      className={cn(
  ...
  return (
    <div className="h-full flex flex-col gap-0">
      {/* Tab bar */}
      <div className="flex items-center gap-1 border-b border-jarvis-border/20 px-4 pt-3 pb-0">
        <TabButton id="builder" label="Builder" icon={LayoutDashboard} />
        <TabButton id="engine" label="Engine" icon={Settings2} />
      </div>
